...
test('expect render with theme', () => {
    ...
    const { getByTestId } = renderWithTheme(<Autocomplete categories={categories} options={options} />, dark)
    ...
    expect(input).toHaveStyleRule('color', dark.field.text.secondary, { modifier: '::placeholder', })
})
...
test('expect to be disabled', () => {
    ...
    const { getByTestId } = renderWithTheme(<Autocomplete categories={categories} options={options} label="Field label" disabled />, light)

    const input = getByTestId('input-autocomplete')
    expect(input).toHaveAttribute('disabled')
    expect(input).toHaveStyleRule('color', light.field.text.disabled, { modifier: '::placeholder', })
